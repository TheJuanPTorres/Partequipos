...
| **Seguridad, leído en 3.89.0 (2026-09-23).** Botón «Pegar URL» visible aunque el endpoint del servidor esté desactivado: el panel solo lo oculta con `pasteURL: false` explícito                                                                                                                                                                                                  | Un botón que invitaba a una función que nadie usa                                                                                                    | `pasteURL: false` en `media` y `videos` (CLAUDE.md §10.32)                                                                                                                                                                                                                                                 |
| **Seguridad, leído en 3.89.0 (2026-09-23).** Una operación de subida SIN fichero y con `data.url` hace que el SERVIDOR descargue esa url (`generateFileData` → `getExternalFile`); en un `create` basta el punto focal por defecto 50/50. Nuestros ganchos de formato y tamaño no lo ven (miran `req.file`)                                                                       | Un editor podía hacer que el lambda cargase en memoria un fichero de cualquier tamaño de cualquier host público, saltándose el tope de 4 MB de vídeo | Gancho `sinDescargaRemota`: solo el host del Blob propio. **DEPENDE de que `beforeOperation` corra antes de `generateFileData` y de que la url venga en `data.url`**: si Payload cambia cualquiera de las dos, el cierre queda sin efecto **sin avisar**. Revisar en cada actualización (CLAUDE.md §10.32) |
| **Datos, observado en 3.89.0 (2026-09-23), CAUSA SIN DEMOSTRAR.** Tras recortar en el panel, el registro dice 600×400 y la URL del Blob siguió sirviendo el original durante al menos 54 s. Puede ser el fichero mal escrito o la caché del CDN (Vercel: hasta 60 s al sobrescribir; y el plugin sube con `max-age` de un año en el mismo nombre). Igual con y sin nuestro gancho | El registro miente sobre el fichero: `next/image` pintaría la imagen con la proporción equivocada                                                    | **Recorte desactivado** (2026-09-23): `crop: false` y el gancho `sinRecorte`, porque Payload recorta por la API aunque `crop: false`. Reabrirlo solo con `addRandomSuffix: true` y probado (CLAUDE.md §10.32)                                                                                              |
